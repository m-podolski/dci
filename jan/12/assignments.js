...
function leqzero(num) {

  return num <= 0 ? true : false;
}

console.log(leqzero(-1));
console.log(leqzero(0));
console.log(leqzero(1));
...
function dogYears(humanYears) {
  ...
  console.log(`Your doggy is ${humanYears * 7} years old in dog years!`);
}
...
function calcLifetimeSupply(currentAge, snacksPerDay) {

  const result = (100 - currentAge) * (snacksPerDay * 356);
  console.log(`The snack company should provide you with ${result} units!`);
}
...
function countOccurrences(string, char) {
  ...
  for (let i = 0; i < string.length; i++) {
    ...
    count = string[i] === char ? count + 1 : count;
  }
  console.log(count);
}
